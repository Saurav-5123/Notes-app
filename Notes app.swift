    ...
    var body: some View {
        NavigationView {
            VStack {
                ...
                .sheet(isPresented: $showingNoteSheet) {
                    // Correctly pass noteName and noteTitle
                    EditItemView(
                        noteName: editingNote?.name ?? "",
                        noteTitle: editingNote?.title ?? "",
                        onSave: { name, title in
                ...
                .sheet(isPresented: $showingReminderSheet) {
                    // Correctly pass title to EditItemView
                    EditItemView(
                        noteName: editingReminder?.title ?? "", // Just using title here
                        noteTitle: "", // Reminder doesn't have noteTitle, leave it empty
                        onSave: { name, _ in
                            if let reminder = editingReminder {
                                if let index = reminders.firstIndex(where: { $0.id == reminder.id }) {
                                    reminders[index].title = name
                                }
                            } else {
                                reminders.append(Reminder(title: name))
                            }
                        }
                    )
                }
    ...
    var body: some View {
        NavigationView {
            Form {
                ...
                TextField("Note Name", text: $noteTitle)
            }
            .navigationTitle("Editor")
            .toolbar {
                ToolbarItem(placement: .confirmationAction) {
